import { noop, hooks, Events, EMPTY_OBJ, isNumber, isString, isArray, isUndefined, warn, isFunction, getComponentsAlias as getComponentsAlias$1, internalComponents, toCamelCase, isObject, ensure, isNull, toDashed, PLATFORM_TYPE, controlledComponent, EventChannel } from '../../shared';
...
const taroGetComputedStyleProvider = "harmony" === 'web' ? env.window.getComputedStyle : function (element) {
...
const TaroURLProvider = "harmony" === 'web' ? env.window.URL : TaroURL;
...
const _raf = "harmony" === 'web' ? requestAnimationFrame : function (callback) {
    const _now = now();
    const nextTime = Math.max(lastTime + 16, _now); // First time will execute it immediately but barely noticeable and performance is gained.
    return setTimeout(function () { callback(lastTime = nextTime); }, nextTime - _now);
};
const _caf = "harmony" === 'web'
...
const taroWindowProvider = "harmony" === 'web' ? env.window : (env.window = new TaroWindow());
...
if ("harmony" !== PLATFORM_TYPE.WEB && "harmony" !== PLATFORM_TYPE.HARMONY) {
...
const taroDocumentProvider = "harmony" === 'web' ? env.document : (env.document = createDocument());
